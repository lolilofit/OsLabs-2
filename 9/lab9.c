...
//pthread_mutex_t cound_mutext[5];
...
int main (int argn,
  ...
  int i;
  //for(i = 0; i < 5; i++)
 // 	pthread_mutex_init (&(cound_mutext[i]), NULL);
  if (argn == 2)
...
//  printf ("Philosopher %d sitting down to dinner.\n", id);
  ...
  while (f = food_on_table ()) {
   ...
   // printf ("Philosopher %d: get dish %d.\n", id, f);
    ...
    get_fork (id, right_fork, "rigth", 1);
...
/*  
  if(is_left == 0) {
//	printf("it's left %d\n", fork);
	printf("try to block %d\n", fork);
  	pthread_mutex_lock(&(cound_mutext[fork]));
//	printf("try to block left %d\n", (fork-1 + 5)%5);
  	pthread_mutex_lock(&(cound_mutext[(fork-1 + 5)%5]));
//	printf("blocked left %d\n", (fork-1+5)%5);
  }
*/
  if(is_left == 0)
...
/*  
 if(is_left != 0) {
//	printf("it's right %d\n", (fork-1+5)%5);
	pthread_mutex_unlock(&(cound_mutext[(fork-2 + 5)%5]));
 //       printf("unlocked left%d\n", (fork-2+5)%5);
	pthread_mutex_unlock(&(cound_mutext[(fork-1+5)%5]));
//	printf("unlocked %d\n", (fork-1+5)%5);
  }
 */
