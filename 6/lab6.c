...
//	struct Param param = *((struct Param*)args);
	...
	DIR* dir = opendir(param[0]);
	struct dirent* buf;
	struct dirent* de;
	long buf_size = sizeof(struct dirent) + pathconf(param[0], _PC_NAME_MAX) + 1;
...
//	recurs_param = (struct Param*)malloc(sizeof(struct Param));
	...
	if(dir != NULL && buf_size > 0) {
		...
		//	if(result == 0) {
				printf("readdir_r succeded\n");
				int elem_size = strlen(param[0]) + 2 + strlen(de->d_name);
				...
				sprintf(elem_path, "%s/%s", param[0], de->d_name);
		...
		//		recurs_param = (struct Param*)realloc(recurs_param, recurs_param_size);
				...
				if(statbuf.st_mode & S_IFDIR) {
					...
					int directory_dst_count = strlen(param[1]) + 2 + strlen(de->d_name);
					...
					sprintf(pass[0], "%s", elem_path);
					sprintf(pass[1], "%s/%s", param[1], de->d_name);

					mkdir(pass[1], permissions(statbuf));

					printf("chectk dir : %s %s \n", pass[0], pass[1]);
					status = pthread_create(&(threads[threads_count-1]), NULL, copy_dir, (void*)(&pass));
                                        if(status != 0)
				...
				if(statbuf.st_mode & S_IFREG) {
					...
					int file_dst_count = strlen(param[1]) + 2 + strlen(de->d_name); 
					char pass[2][257];
					//recurs_param[recurs_param_size - 1].path_src = (char*)malloc(sizeof(char)*(strlen(elem_path)+1));
					...
					status = pthread_create(&(threads[threads_count-1]), NULL, copy_file, (void*)(&pass));
				        if(status != 0)
                				fprintf(stderr, "Error creating file copy thread\n");
				}
...
int main(int argc, char* argv[]) {
	...
	char param[257][257];
	sprintf(param[0], "%s", argv[1]);
	sprintf(param[1], "%s", argv[2]);
