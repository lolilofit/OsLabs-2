	...
	if(dir != NULL && buf_size > 0) {
		...
		//	if(result == 0) {
				...
				if(statbuf.st_mode & S_IFDIR) {
					...
					sprintf(recurs_param[recurs_param_size - 1].path_src, "%s", elem_path);
					sprintf(recurs_param[recurs_param_size - 1].path_dst, "%s/%s", param.path_dst, de->d_name);

					mkdir(recurs_param[recurs_param_size - 1].path_dst, permissions(statbuf));

					printf("chectk dir : %s %s \n", recurs_param[recurs_param_size - 1].path_src, recurs_param[recurs_param_size - 1].path_dst);
					status = pthread_create(&(threads[threads_count-1]), NULL, copy_dir, (void*)(&recurs_param[recurs_param_size - 1]));
                                        if(status != 0)
				...
				if(statbuf.st_mode & S_IFREG) {
					...
					status = pthread_create(&(threads[threads_count-1]), NULL, copy_file, (void*)(&recurs_param[recurs_param_size - 1]));
				        if(status != 0)
                				fprintf(stderr, "Error creating file copy thread\n");
				}
